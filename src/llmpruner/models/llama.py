# code was modified from HF github repo
...
from llmpruner.model_utils import prune_embedding_layer
...
class LlamaMLP(nn.Module):
    ...
    def _prune_neurons(self, index):
        self.gate_proj = prune_linear_layer(self.gate_proj, index)
        self.up_proj = prune_linear_layer(self.up_proj, index)
        self.down_proj = prune_linear_layer(self.down_proj, index, dim=1)

    def forward(self, x):
        intermediary_output = self.gate_proj(x)
        down_proj = self.down_proj(self.act_fn(intermediary_output)*self.up_proj(x))

        return down_proj, intermediary_output
...
class LlamaAttention(nn.Module):
    ...
    def _prune_embedding(self, index):        
        ...
        self.o_proj = prune_linear_layer(self.o_proj, index)
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output_pruning = torch.matmul(attn_weights, value_states)

        if attn_output_pruning.size() != (bsz, self.num_heads, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads, q_len, self.head_dim)}, but is"
                f" {attn_output.size()}"
            )

        attn_output = attn_output_pruning.transpose(1, 2).contiguous()
        ...
        return attn_output, attn_weights, past_key_value, attn_output_pruning
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output_pruning = torch.nn.functional.scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
        ...
        return attn_output, None, past_key_value, attn_output_pruning
    ...
    def prune_heads(self, heads_to_prune):
        for module in self.modules():
            if isinstance(module, LlamaAttention):
                module._prune_heads(heads_to_prune)
        
    def prune_neurons(self, index_to_keep):        
        for module in self.modules():
            if isinstance(module, LlamaMLP) :
                module._prune_neurons(index_to_keep)
        
    def prune_embeddings(self, index_to_keep):
        self.embed_tokens = prune_embedding_layer(self.embed_tokens, index_to_keep, dim=1)    
        
        for module in self.modules():
            if isinstance(module, LlamaAttention) or isinstance(module, LlamaMLP) or isinstance(module, LlamaRMSNorm):
                module._prune_embedding(index_to_keep)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prune_neurons(self, index_to_keep):        
        self.model.prune_neurons(index_to_keep)
    
    def prune_embeddings(self, index_to_keep):
        self.model.prune_embeddings(index_to_keep)
        
        # prune the lm head
        self.lm_head = prune_linear_layer(self.lm_head, index_to_keep, dim=1)  
        # tie to the embedding
